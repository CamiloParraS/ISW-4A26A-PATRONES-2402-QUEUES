...
function move(dir) {
    ...
    if (dir === "w" && playerY > 0)
        playerY--;
    if (dir === "s" && playerY < WORLD_SIZE - 1)
        playerY++;
    if (dir === "a" && playerX > 0)
        playerX--;
    if (dir === "d" && playerX < WORLD_SIZE - 1)
        playerX++;
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        process.stdin.on("keypress", (_str, key) => {
            ...
            if (!key)
                return;
            const k = (_a = key.name) === null || _a === void 0 ? void 0 : _a.toLowerCase();
            if (k === "q" || (key.ctrl && k === "c")) {
                console.log("\nGoodbye!");
                process.exit(0);
            }
            if (["w", "a", "s", "d"].includes(k !== null && k !== void 0 ? k : "")) {
                move(k);
            }
            processQueue();
            render();
